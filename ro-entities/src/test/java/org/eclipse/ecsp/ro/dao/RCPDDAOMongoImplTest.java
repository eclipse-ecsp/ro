...
public class RCPDDAOMongoImplTest extends CommonTestBase {
    ...
    private static final String vehicleId = "123456";
    private static final String userId = "abc";
    private static final String requestId = "74125911-7a89-41be-b3e6-831b5fca27a4";
    private static final String scheduleId = "s12ad11-33dff";
    ...
    public void getRCPDRequest() throws IOException {
        saveRCPDRequest();
        Optional<RCPD> result = rcpdDAO.getRCPDRequest(vehicleId, requestId, null);
        Assert.assertTrue(result.isPresent());
        Optional<RCPD> resultWithCcheduleId = rcpdDAO.getRCPDRequest(vehicleId, null, scheduleId);
    ...
    public void getRCPDRequestWithScheduleId() throws IOException {
        saveRCPDRequest();
        Optional<RCPD> result = rcpdDAO.getRCPDRequest(vehicleId, requestId, "s12ad11-33dff");
        Assert.assertTrue(result.isPresent());
    ...
    public void prepareIgniteQueryForRcpdRequest() {
        Assert.assertNotNull(rcpdDAO.prepareIgniteQueryForRcpdRequest(requestId, vehicleId));
    }
    ...
    private void saveRCPDRequest() {
        ...
        data.setScheduleRequestId(scheduleId);
        data.setRcpdRequestId(requestId);
