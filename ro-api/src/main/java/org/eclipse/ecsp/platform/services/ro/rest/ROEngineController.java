...
import org.eclipse.ecsp.platform.services.ro.service.ROEngineService;
...
/**
 * Controller Class for Ro Engine operations.
 *
 * @author midnani
 */
@RestController
public class ROEngineController {

    private static final IgniteLogger LOGGER = IgniteLoggerFactory.getLogger(ROEngineController.class);

    @Autowired
    private ROEngineService roEngineService;

    /**
     * Create remote engine request Api.
     *
     ...
     */
    @Operation(summary = "PUT /v1.1/users/{userId}/vehicles/{vehicleId}/ro/engine",
            description = "create Remote Engine Request",
            responses = {
    ...
    @PutMapping(
            value = "/v1.1/users/{userId}/vehicles/{vehicleId}/ro/engine",
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<RemoteOperationResponse> createRemoteEngineRequest(
            ...
            @RequestBody RemoteOperationEngineReq remoteOperationEngineReq)
            throws
            InterruptedException,
            ExecutionException {
        if (!requestId.isEmpty()) {
            remoteOperationEngineReq.setRoRequestId(requestId);
        }
        LOGGER.info("Received  remoteOperationEngineReq :{}, userId :{} ,vehicleId :{} ,"
                        + "ecuType :{} ,partnerId :{} ,vehicleArchType :{}",
                Utils.logForging(remoteOperationEngineReq),
                Utils.logForging(userId),
                Utils.logForging(vehicleId),
                Utils.logForging(ecuType),
                Utils.logForging(partnerId),
                Utils.logForging(vehicleArchType));

        RemoteOperationResponse remoteOperationResponse = roEngineService
                .createRemoteEngineRequest(userId,
                        vehicleId,
                        remoteOperationEngineReq,
                        sessionId,
                        origin,
                        ecuType,
                        partnerId,
    ...
    /**
     * Create remote engine request v2 Api.
     *
     * @param clientRequestId     the client request id
     * @param requestId           the request id
     * @param sessionId           the session id
     ...
     */
    @Operation(summary = "PUT /v2/users/{userId}/vehicles/{vehicleId}/ro/engine",
            description = "create Remote Engine Request v2",
            responses = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "202", description = "Accepted",
                            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                                    schema = @Schema(implementation = RemoteOperationScheduleResponse.class)))
            }
    )
    @SecurityRequirement(name = Security.JWT_AUTH_VALIDATOR,
            scopes = {"ROSupportOwner, ROSupportThirdParty, SelfManage, ROSupportLawEnforcement, "
    ...
    @Counted(name = "showAll-counted")
    @PutMapping(
            value = "/v2/users/{userId}/vehicles/{vehicleId}/ro/engine",
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<RemoteOperationScheduleResponse> createRemoteEngineRequest_v2(
            @RequestHeader(value = "ClientRequestId", required = false) String clientRequestId,
            @RequestHeader(value = "RequestId") String requestId,
            @RequestHeader(value = "SessionId") String sessionId,
            @RequestHeader(value = "OriginId") String origin,
            ...
            @PathVariable("userId") @Parameter(description = "userId") String userId,
            @PathVariable("vehicleId") @Parameter(description = "vehicleId") String vehicleId,
            @RequestBody RemoteOperationEngineScheduleReq remoteEngineRequest)
            throws
            InterruptedException,
            ExecutionException,
            ForbiddenException,
            BadRequestException {
        if (!requestId.isEmpty()) {
            remoteEngineRequest.setRoRequestId(requestId);
        }
        LOGGER.info("Received remoteEngineRequest :{}, userId :{} ,vehicleId :{} ,ecuType :{} ,vehicleArchType :{}",
                Utils.logForging(remoteEngineRequest),
                Utils.logForging(userId),
                Utils.logForging(vehicleId),
                Utils.logForging(ecuType));

        RemoteOperationScheduleResponse remoteOperationResponse = roEngineService.createRemoteEngineScheduleRequest(
                userId,
                vehicleId,
                remoteEngineRequest,
                sessionId,
        ...
        return new ResponseEntity<RemoteOperationScheduleResponse>(remoteOperationResponse,
                ApiUtils.getHeaders(clientRequestId, sessionId, requestId),
                HttpStatus.ACCEPTED);
    }
}
