...
import org.eclipse.ecsp.platform.services.ro.service.ROClimateService;
...
/**
 * Controller class for RO Climate Api.
 *
 * @author midnani
 */
@RestController
public class ROClimateController {

    private static final IgniteLogger LOGGER = IgniteLoggerFactory.getLogger(ROClimateController.class);

    @Autowired
    private ROClimateService roClimateService;
     ...
     */
    @Operation(summary = "PUT /v1.1/users/{userId}/vehicles/{vehicleId}/ro/climate",
            description = "create Remote Climate Request",
            responses = {
    ...
    @PutMapping(value = "/v1.1/users/{userId}/vehicles/{vehicleId}/ro/climate",
            consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<RemoteOperationResponse> createRemoteClimateRequest(
            ...
            @RequestBody RemoteOperationClimateReq remoteOperationClimateReq)
            throws InterruptedException, ExecutionException, JsonProcessingException {
        if (!requestId.isEmpty()) {
            remoteOperationClimateReq.setRoRequestId(requestId);
        }
        LOGGER.info("Received remoteOperationClimateReq :{}, userId :{} ,vehicleId :{}"
                        + " ,partnerId :{} ,ecuType :{} ,vehicleArchType :{}",
                Utils.logForging(remoteOperationClimateReq), Utils.logForging(userId),
                Utils.logForging(vehicleId), Utils.logForging(partnerId),
                Utils.logForging(ecuType), Utils.logForging(vehicleArchType));
        RemoteOperationResponse remoteOperationResponse = roClimateService
                .createRemoteClimateRequest(userId, vehicleId,
                        remoteOperationClimateReq, sessionId, origin, partnerId, ecuType, vehicleArchType);
     ...
     * @throws JsonProcessingException the json processing exception
     */
    @Operation(summary = "PUT /v2/ro/hvac",
            description = "create Remote HVAC Request",
            responses = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "202", description = "Accepted",
                            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                                    schema = @Schema(implementation = RemoteOperationResponse.class)))
    ...
    @ExceptionMetered
    @Counted(name = "showAll-counted")
    @PutMapping(value = "/v2/ro/hvac", consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<RemoteOperationResponse> createRemoteHVACRequest(
            @RequestHeader(value = "ClientRequestId", required = false) String clientRequestId,
            @RequestHeader(value = "RequestId") String requestId,
            @RequestHeader(value = "SessionId") String sessionId,
            @RequestHeader(value = "OriginId") String origin,
            ...
            @RequestHeader(value = EventAttribute.PARTNER_ID, required = false) String partnerId,
            @RequestHeader(value = "ecuType", required = false) String ecuType,
            @RequestHeader(value = "vehicleArchType", required = false)
            String vehicleArchType,
            @RequestBody @Valid RemoteOperationClimateReqV2_0 remoteClimateRequest)
            throws InterruptedException, ExecutionException, JsonProcessingException {
        if (!requestId.isEmpty()) {
            remoteClimateRequest.setRoRequestId(requestId);
        }
        LOGGER.info("Received remoteClimateRequest :{}, "
                        + "userId :{} ,vehicleId :{} ,partnerId :{} ,ecuType :{} ,vehicleArchType :{}",
                Utils.logForging(remoteClimateRequest), Utils.logForging(userId),
                Utils.logForging(vehicleId), Utils.logForging(partnerId),
                Utils.logForging(vehicleArchType));
        RemoteOperationResponse remoteOperationResponse = roClimateService
                .createRemoteHVACRequest(userId, vehicleId,
                        remoteClimateRequest, sessionId, origin, partnerId, ecuType, vehicleArchType);
        return new ResponseEntity<RemoteOperationResponse>(remoteOperationResponse,
                ApiUtils.getHeaders(clientRequestId, sessionId, requestId),
                HttpStatus.ACCEPTED);
    }
