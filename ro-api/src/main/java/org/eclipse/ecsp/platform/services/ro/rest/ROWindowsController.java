...
import org.eclipse.ecsp.platform.services.ro.service.ROWindowsService;
 ...
 */
@RestController
public class ROWindowsController {

    private static final IgniteLogger LOGGER = IgniteLoggerFactory.getLogger(ROWindowsController.class);

    @Autowired
    private ROWindowsService roWindowsService;
     ...
     * @param remoteWindowsRequest the remote windows request
     * @return the response entity
     * @throws InterruptedException the interrupted exception
     * @throws ExecutionException   the execution exception
     */
    @Operation(summary = "PUT /v1.1/users/{userId}/vehicles/{vehicleId}/ro/windows",
            description = "create Remote Window Request",
            responses = {
    ...
    @PutMapping(value = "/v1.1/users/{userId}/vehicles/{vehicleId}/ro/windows",
            consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<RemoteOperationResponse> createRemoteWindowRequest(
            ...
            @RequestBody RemoteOperationWindowsReq remoteWindowsRequest)
            throws InterruptedException, ExecutionException {
        if (!requestId.isEmpty()) {
            remoteWindowsRequest.setRoRequestId(requestId);
        }
        LOGGER.info("Received remoteWindowRequest :{}, userId :{} ,vehicleId :{} "
                        + ",ecuType :{} ,vehicleArchType :{}",
                Utils.logForging(remoteWindowsRequest), Utils.logForging(userId), Utils.logForging(vehicleId),
                Utils.logForging(ecuType),
                Utils.logForging(vehicleArchType));
        RemoteOperationResponse remoteOperationResponse = roWindowsService.createRemoteWindowsRequest(userId, vehicleId,
                remoteWindowsRequest, sessionId, origin, ecuType, vehicleArchType);
     ...
     * @throws ExecutionException   the execution exception
     */
    @Operation(summary = "PUT /v1.1/users/{userId}/vehicles/{vehicleId}/ro/windows/driver",
            description = "create Remote Driver Window Request",
            responses = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "202", description = "Accepted",
                            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                                    schema = @Schema(implementation = RemoteOperationResponse.class)))
    ...
    @ExceptionMetered
    @Counted(name = "showAll-counted")
    @PutMapping(value = "/v1.1/users/{userId}/vehicles/{vehicleId}/ro/windows/driver",
            consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<RemoteOperationResponse> createRemoteDriverWindowRequest(
            @RequestHeader(value = "ClientRequestId", required = false) String clientRequestId,
            @RequestHeader(value = "RequestId") String requestId,
            @RequestHeader(value = "SessionId") String sessionId,
            @RequestHeader(value = "OriginId") String origin,
            ...
            @PathVariable("userId") @Parameter(description = "userId") String userId,
            @PathVariable("vehicleId") @Parameter(description = "vehicleId") String vehicleId,
            @RequestBody RemoteOperationDriverWindowReq remoteOperationDriverWindowReq)
            throws InterruptedException, ExecutionException {
        if (!requestId.isEmpty()) {
            remoteOperationDriverWindowReq.setRoRequestId(requestId);
        }
        LOGGER.info("Received remoteOperationDriverWindowReq :{},"
                        + " userId :{} ,vehicleId :{}  ,ecuType :{}, ,vehicleArchType :{}",
                Utils.logForging(remoteOperationDriverWindowReq), Utils.logForging(userId),
                Utils.logForging(vehicleId), Utils.logForging(ecuType),
                Utils.logForging(vehicleArchType));
        RemoteOperationResponse remoteOperationResponse = roWindowsService
                .createRemoteDriverWindowRequest(userId, vehicleId,
                        remoteOperationDriverWindowReq, sessionId, origin, ecuType, vehicleArchType);
        return new ResponseEntity<RemoteOperationResponse>(remoteOperationResponse,
                ApiUtils.getHeaders(clientRequestId, sessionId, requestId),
                HttpStatus.ACCEPTED);
    }
