...
/**
 * ROStatusHistoryController: test class.
 *
 * @author midnani
 */
...
public class ROStatusHistoryControllerTest extends CommonTestBase {

    private static final IgniteLogger LOGGER = IgniteLoggerFactory.getLogger(ROStatusHistoryControllerTest.class);
    ...
    public void testGetRemoteOpHistory_verifyDefaultLimit() throws IOException {
        ...
        ResponseEntity<String> response = restTemplate.exchange(API_VERSION
                        + "/users/testdata/vehicles/12345678/ro/history",
                HttpMethod.GET, entity, String.class);
        Assert.assertNotNull(response.getBody());
        JsonNode rootNode = mapper.readTree(response.getBody());
    ...
    public void testGetRemoteOpHistory_verifyFetchLimit() throws IOException {
        ...
        Assert.assertEquals(TestConstants.TIME_STAMP, remoteHistoryData1.get("roEvent").get("Timestamp").asLong());
    ...
    private void populateRemoteOperationsHistory() throws IOException {
        String requestJson = IOUtils.toString(
                ROStatusHistoryControllerTest.class.getResourceAsStream("/remoteHistoryRequest_1.json"), "UTF-8");
    ...
    private void populateRemoteOperationsStatus() throws IOException {
        String requestJson = IOUtils.toString(
                ROStatusHistoryControllerTest.class.getResourceAsStream("/remoteStatusRequest_1.json"), "UTF-8");
        Ro roStatusData = mapper.readValue(requestJson, Ro.class);
        roDAOMongoImpl.save(roStatusData);
        requestJson = IOUtils.toString(
                ROStatusHistoryControllerTest.class.getResourceAsStream("/remoteStatusRequest_2.json"), "UTF-8");
    ...
    private void populate10RemoteOperationsHistory() throws IOException {
        String requestJson = IOUtils.toString(
                ROStatusHistoryControllerTest.class.getResourceAsStream("/remoteHistoryRequest_10_vehicles.json"),
                "UTF-8");
    ...
    private HttpHeaders createHeadersAdditional() {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        headers.add("ClientRequestId", "testGlobalTrackingId");
        headers.add("RequestId", "testRequestId");
        ...
        headers.add("vehicleId", "123");
        headers.add("roRequestId", "kmpxNPUwqEW69CzK7VmO98BLWpLOqSw1");
        return headers;
    }
}
